...
class Sql extends BaseModel {
  ...
  Future<Map<String, dynamic>> update(Map<String, dynamic> json) async{
    var id = json.remove("id");
    this.db.update(tableName, json, where: "id = ?", whereArgs: [id]);
    json['id'] = id;
    return json;
  }

  Future<int> set(Map<String, dynamic> values,String where, List<dynamic> whereArgs) async{
    int i = await this.db.update(tableName, values, where:where, whereArgs: whereArgs);

    return i;
  }

  Future<void> execute(sql, [List<dynamic> arguments]) async{
    await this.db.execute(sql, arguments);
  }
  ///
  /// 搜索
